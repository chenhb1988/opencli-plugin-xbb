...
cli({
  ...
  func: async function (kwargs) {
    ...
    return list.slice(0, Number(kwargs.limit || 20)).map((item, index) => ({ rank: index + 1, dataId: item.dataId || '', formId: item.formId || '', alone: item.alone || '', uuid: item.uuid || '', addTime: item.addTime || '', updateTime: item.updateTime || '', data: JSON.stringify(item.data || {}), code: '', msg: '', requestBody: debug ? body : '', responseBody: '' }));
  },
});
