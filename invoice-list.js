...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/invoice/list';
...
cli({
  site: 'xbb',
  name: 'invoice-list',
  description: '销项发票列表接口（businessType: 901）',
  strategy: Strategy.PUBLIC,
  access: 'read',
  browser: false,
  domain: 'proapi.xbongbong.com',
  args: [
    { name: 'listGroupId', type: 'str', default: '', help: '分组类型：正常销项发票61，红冲销项发票62，作废红冲发票201（可选）' },
  ...
  func: async function (kwargs) {
    ...
    if (String(kwargs.viewApproval ?? '') !== '') payload.viewApproval = Number(kwargs.viewApproval);
