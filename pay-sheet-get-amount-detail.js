...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/paySheet/getAmountDetail';
...
function makeErrorRow(code, msg) {
  return [{ rank: '', amountDetail: '', code, msg }];
}

cli({
  site: 'xbb',
  name: 'pay-sheet-get-amount-detail',
  description: '付款单金额明细接口（businessType: 802）',
  strategy: Strategy.PUBLIC,
  access: 'read',
  browser: false,
  domain: 'proapi.xbongbong.com',
  args: [
    { name: 'dataId', type: 'int', help: '付款单id（必填）' },
    { name: 'userId', type: 'str', default: '', help: '操作人id（可选）' },
    { name: 'debug', type: 'bool', default: false, help: '输出请求体和返回体调试信息' },
  ],
  columns: ['rank', 'amountDetail', 'code', 'msg'],
  func: async (kwargs) => {
    ...
    if (!amountDetail.length) return makeErrorRow('NO_DATA', '接口成功，但 amountDetail 为空');
    return amountDetail.map((item, index) => ({ rank: index + 1, amountDetail: JSON.stringify(item || {}), code: '', msg: '' }));
  },
});
