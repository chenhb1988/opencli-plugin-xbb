...
const REFUND_LIST_API_URL = 'https://proapi.xbongbong.com/pro/v2/api/refund/list';
...
function makeSuccessRows(list, debug, body, kwargs) {
  ...
  return list.slice(0, limit).map((item, index) => ({
    ...
    customerId: item.data?.text_1 || '',
    contractId: item.data?.text_2 || '',
    refundType: item.data?.text_4 || '',
    refundAmount: item.data?.num_1 || '',
...
cli({
  site: 'xbb',
  name: 'refundlist',
  description: '退货退款单列表接口（纯 HTTP 版）',
  ...
  columns: ['rank', 'dataId', 'formId', 'serialNo', 'customerId', 'contractId', 'refundType', 'refundAmount', 'creatorId', 'addTime', 'updateTime', 'data', 'code', 'msg', 'requestBody', 'responseBody'],
  func: async function (_page, kwargs) {
    ...
    const resp = await fetch(buildApiUrl(baseUrl, REFUND_LIST_API_URL), {
      method: 'POST',
      headers: {
