...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/quotation/deleteMainUser';
...
cli({
  site: 'xbb',
  name: 'quotation-delete-mainuser',
  description: '报价单删除负责人接口（businessType: 4700）',
  ...
  args: [
    ...
    { name: 'userIds', type: 'str', help: '需删除的负责人id列表，JSON数组字符串（必填）' },
  ...
  func: async function (kwargs) {
    ...
    const parsedUserIds = parseJsonArray(kwargs.userIds);
    if (Array.isArray(parsedUserIds)) payload.userIds = parsedUserIds;
    ...
    if (parsedUserIds === null) return makeErrorRow('NO_USERIDS', '缺少 --userIds');
    if (parsedUserIds === undefined) return makeErrorRow('INVALID_USERIDS', '--userIds 必须是 JSON 数组字符串');
