...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/workOrderV2/asset/addMinUser';
...
cli({
  site: 'xbb',
  name: 'work-order-asset-add-mainuser',
  description: '资产添加负责人接口（businessType: -1；workOrderV2 资产）',
  ...
  args: [
    ...
    { name: 'dataId', type: 'int', help: '资产id（必填）' },
    { name: 'businessUserIdList', type: 'str', help: '负责人id列表，JSON数组字符串（必填）' },
  ...
  func: async function (kwargs) {
    ...
    const parsedBusinessUserIdList = parseJsonArray(kwargs.businessUserIdList);
    if (Array.isArray(parsedBusinessUserIdList)) payload.businessUserIdList = parsedBusinessUserIdList;
    ...
    if (parsedBusinessUserIdList === null) return makeErrorRow('NO_BUSINESSUSERIDLIST', '缺少 --businessUserIdList');
    if (parsedBusinessUserIdList === undefined) return makeErrorRow('INVALID_BUSINESSUSERIDLIST', '--businessUserIdList 必须是 JSON 数组字符串');
    ...
    return [{ resultCode: data.code ?? '', resultMsg: data.msg || '', messageList: JSON.stringify(result.messageList || []), code: data.code ?? '', msg: data.msg || '' }];
  },
});
