...
function makeErrorRow(code, msg, debug, requestBody = '', responseBody = '') {
  return [{ dataId: '', type: '', title: '', taxpayerNo: '', registeredPhone: '', invoiceAddress: '', bank: '', account: '', code, msg, requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
}
...
cli({
  ...
  columns: ['dataId', 'type', 'title', 'taxpayerNo', 'registeredPhone', 'invoiceAddress', 'bank', 'account', 'code', 'msg', 'requestBody', 'responseBody'],
  func: async function (kwargs) {
    ...
    if (!list.length) return [{ dataId: '', type: '', title: '', taxpayerNo: '', registeredPhone: '', invoiceAddress: '', bank: '', account: '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
