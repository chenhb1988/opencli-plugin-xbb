...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/product/categoryUpdate';
...
function getValidationError(payload, token) {
  ...
  if (!payload.name) {
    return { code: 'NO_NAME', msg: '缺少 --name' };
  }
  if (!token) {
...
function makeSuccessRow(payload, data, debug, requestBody, responseBody) {
  const result = data.result || {};
  return [{
    dataId: result.dataId || payload.dataId || '',
    code: data.code ?? '',
    msg: data.msg || '',
    requestBody: debug ? requestBody : '',
    responseBody: debug ? responseBody : '',
...
cli({
  ...
  func: async function (kwargs) {
    ...
    return makeSuccessRow(payload, data, debug, requestBody, responseBody);
  },
});
