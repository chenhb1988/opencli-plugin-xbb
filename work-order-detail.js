...
const WORK_ORDER_DETAIL_API_URL = 'https://proapi.xbongbong.com/pro/v2/api/workOrderV2/detail';
...
function buildPayload(kwargs) {
  ...
  const queryFlag = parseOptionalInteger(kwargs.queryFlag, 'INVALID_QUERY_FLAG', 'queryFlag 必须是数字');
  if (queryFlag !== null) {
...
function makeSuccessRow(data, debug, requestBody, responseBody) {
  ...
  return [{
    ...
    ownerId: stringifyValue(result.data?.ownerId),
    coUserId: stringifyValue(result.data?.coUserId),
