...
function makeErrorRow(code, msg, debug, requestBody = '', responseBody = '') {
  return [{ dataId: '', taskName: '', planStartTime: '', planEndTime: '', remark: '', code, msg, requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
}
...
cli({
  ...
  columns: ['dataId', 'taskName', 'planStartTime', 'planEndTime', 'remark', 'code', 'msg', 'requestBody', 'responseBody'],
  func: async (kwargs) => {
    ...
    return [{ dataId: result.dataId || '', taskName: result.taskName || '', planStartTime: result.planStartTime || '', planEndTime: result.planEndTime || '', remark: result.remark || '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
  },
});
