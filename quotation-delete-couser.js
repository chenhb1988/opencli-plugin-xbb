...
cli({
  site: 'xbb',
  name: 'quotation-delete-couser',
  description: '报价单删除协同人接口（businessType: 4700）',
  ...
  args: [
    ...
    { name: 'coUserIds', type: 'str', help: '需删除的协同人id列表，JSON数组字符串（必填）' },
  ...
  func: async function (kwargs) {
    ...
    const parsedCoUserIds = parseJsonArray(kwargs.coUserIds);
    if (Array.isArray(parsedCoUserIds)) payload.coUserIds = parsedCoUserIds;
    ...
    if (parsedCoUserIds === null) return makeErrorRow('NO_COUSERIDS', '缺少 --coUserIds');
    if (parsedCoUserIds === undefined) return makeErrorRow('INVALID_COUSERIDS', '--coUserIds 必须是 JSON 数组字符串');
