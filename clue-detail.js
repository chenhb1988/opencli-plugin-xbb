...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/clue/detail';
...
function makeErrorRow(code, msg, debug, requestBody = '', responseBody = '') {
  return [{ dataId: '', formId: '', addTime: '', updateTime: '', data: '', code, msg, requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
}
...
cli({
  ...
  func: async function (kwargs) {
    ...
    return [{ dataId: result.dataId || '', formId: result.formId || '', addTime: result.addTime || '', updateTime: result.updateTime || '', data: JSON.stringify(result.data || {}), code: data.code ?? '', msg: data.msg || '', requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
  },
});
