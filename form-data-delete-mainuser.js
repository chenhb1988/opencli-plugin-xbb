...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/paas/deleteMainUser';
...
function makeErrorRow(code, msg, debug, body = '', responseBody = '') {
  return [{ resultCode: '', resultMsg: '', messageList: '', code, msg, requestBody: debug ? body : '', responseBody: debug ? responseBody : '' }];
}

cli({
  site: 'xbb',
  name: 'form-data-delete-mainuser',
  description: '自定义表单删除负责人接口',
  strategy: Strategy.PUBLIC,
  access: 'write',
  browser: false,
  domain: 'proapi.xbongbong.com',
  args: [
    { name: 'dataId', type: 'int', help: '表单id（必填）' },
  ...
  func: async (kwargs) => {
    ...
    if (!Array.isArray(businessUserIdList) || !businessUserIdList.length) return makeErrorRow('NO_USERIDLIST', '缺少 --businessUserIdList 或格式不正确，需为JSON数组', debug, body, '');
    ...
    return [{ resultCode: data.code ?? '', resultMsg: data.msg || '', messageList: JSON.stringify(data.result?.messageList || []), code: data.code ?? '', msg: data.msg || '', requestBody: debug ? body : '', responseBody: debug ? responseBody : '' }];
  },
});
