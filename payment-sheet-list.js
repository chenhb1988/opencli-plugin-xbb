...
function buildPayload(kwargs, corpid) {
  ...
  const listGroupId = Number(kwargs.listGroupId);
  if (Number.isFinite(listGroupId) && listGroupId > 0) {
    payload.listGroupId = listGroupId;
  }

  const subBusinessType = Number(kwargs.subBusinessType);
  if (Number.isFinite(subBusinessType) && subBusinessType > 0) {
    payload.subBusinessType = subBusinessType;
  }
...
cli({
  ...
  columns: ['rank', 'dataId', 'formId', 'serialNo', 'customerIds', 'contractIds', 'receivableIds', 'receivedAmount', 'ownerId', 'creatorId', 'addTime', 'updateTime', 'data', 'code', 'msg'],
  func: async function (kwargs) {
    ...
    if (debug) {
      process.stderr.write(`[debug] URL: ${buildApiUrl(baseUrl, PAYMENT_SHEET_LIST_API_URL)}\n[debug] Headers: ${JSON.stringify(headers)}\n[debug] RequestBody: ${body}\n`);
    }
