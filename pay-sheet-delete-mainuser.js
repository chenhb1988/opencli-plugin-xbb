...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/paySheet/deleteMainUser';
...
cli({
  site: 'xbb',
  name: 'pay-sheet-delete-mainuser',
  description: '付款单删除负责人接口',
  ...
  func: async (kwargs) => {
    ...
    if (!token) return makeErrorRow('NO_TOKEN', '缺少 token；请先执行 opencli xbb token-set --corpid <CORPID> --token <TOKEN> --userId <USERID>', debug, requestBody, '');
