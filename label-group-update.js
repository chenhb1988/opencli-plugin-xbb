...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/label/group/update';
...
function makeErrorRow(code, msg) {
  return [{ groupId: '', code, msg }];
}

cli({
  site: 'xbb',
  name: 'label-group-update',
  description: '编辑标签分组接口（businessType: -1；label 模块）',
  ...
  func: async function (kwargs) {
    ...
    return [{ groupId: result.groupId || result.id || '', code: data.code ?? '', msg: data.msg || '' }];
  },
});
