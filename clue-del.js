...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/clue/del';
...
function makeErrorRow(code, msg, debug, requestBody = '', responseBody = '') {
  return [{ errorDataMemo: '', code, msg, requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
}
...
cli({
  ...
  columns: ['errorDataMemo', 'code', 'msg', 'requestBody', 'responseBody'],
  func: async function (kwargs) {
    ...
    return [{ errorDataMemo: data.result?.errorDataMemo || '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
  },
});
