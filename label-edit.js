...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/label/edit';
...
cli({
  site: 'xbb',
  name: 'label-edit',
  description: '编辑标签接口（businessType: -1；label 模块）',
  ...
  args: [
    ...
    { name: 'dataId', type: 'int', help: '标签id（必填）' },
    { name: 'color', type: 'str', help: '标签颜色（必填）' },
    { name: 'name', type: 'str', default: '', help: '标签名（可选）' },
  ...
  func: async function (kwargs) {
    ...
    if (String(kwargs.name || '').trim()) payload.name = String(kwargs.name).trim();
