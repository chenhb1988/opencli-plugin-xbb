...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/customer/invoice/addressEdit';
...
function makeErrorRow(code, msg, debug, requestBody = '', responseBody = '') {
  return [{ addressId: '', resultCode: '', resultMsg: '', code, msg, requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
}

cli({
  site: 'xbb',
  name: 'customer-invoice-address-edit',
  description: '编辑客户开票地址接口',
  ...
  args: [
    ...
    { name: 'addressId', type: 'int', help: '开票地址id（必填）' },
    { name: 'dataList', type: 'str', help: '开票地址JSON字符串（必填）' },
    { name: 'userId', type: 'str', default: '', help: '操作人id（可选）' },
    { name: 'debug', type: 'bool', default: false, help: '输出请求体和返回体调试信息' },
  ],
  columns: ['addressId', 'resultCode', 'resultMsg', 'code', 'msg', 'requestBody', 'responseBody'],
  func: async function (kwargs) {
    const debug = Boolean(kwargs.debug);
    const { configCorpid, token, baseUrl, userId } = getRuntimeConfig();
    const parsedDataList = parseDataList(kwargs.dataList);
    const payload = { corpid: String(kwargs.corpid || ''), customerId: Number(kwargs.customerId || 0), addressId: Number(kwargs.addressId || 0) };
    ...
    if (!payload.addressId) return makeErrorRow('NO_ADDRESSID', '缺少 --addressId', debug, requestBody, '');
    ...
    return [{ addressId: result.addressId || payload.addressId || '', resultCode: result.code ?? '', resultMsg: result.msg || '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
  },
});
