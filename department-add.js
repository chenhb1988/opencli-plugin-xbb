...
function makeErrorRow(code, msg, debug, requestBody = '', responseBody = '') {
  return [{ id: '', code, msg, requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
}
...
cli({
  ...
  columns: ['id', 'code', 'msg', 'requestBody', 'responseBody'],
  func: async function (kwargs) {
    const debug = Boolean(kwargs.debug);
    const { configCorpid, token, baseUrl, userId } = getRuntimeConfig();
    const payload = { name: String(kwargs.name || ''), parentId: Number(kwargs.parentId || 0), corpid: String(kwargs.corpid || '') };
    ...
    if (!payload.parentId) return makeErrorRow('NO_PARENTID', '缺少 --parentId', debug, requestBody, '');
    ...
    return [{ id: data.result?.id || '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
  },
});
