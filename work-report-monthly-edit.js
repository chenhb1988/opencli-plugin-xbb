...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/workReportMonthly/edit';
...
cli({
  ...
  func: async (kwargs) => {
    ...
    return [{ dataId: result.dataId || '', resultCode: data.code ?? '', resultMsg: data.msg || '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? body : '', responseBody: debug ? responseBody : '' }];
  },
});
