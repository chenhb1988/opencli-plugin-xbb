...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/clue/handover';
...
cli({
  site: 'xbb',
  name: 'clue-handover',
  description: '线索移交接口（businessType: 8000）',
  strategy: Strategy.PUBLIC,
  access: 'write',
  browser: false,
  domain: 'proapi.xbongbong.com',
  args: [
    { name: 'dataIdList', type: 'str', help: '线索id列表，JSON数组字符串，最多200条（必填）' },
    { name: 'businessUserId', type: 'str', help: '移交目标人员id（必填）' },
    { name: 'originUserId', type: 'str', default: '', help: '被移交用户，隔离模式移交线索时必填（可选）' },
  ...
  func: async function (kwargs) {
    ...
    payload.businessUserId = String(kwargs.businessUserId || '').trim();
    if (String(kwargs.originUserId || '').trim()) payload.originUserId = String(kwargs.originUserId).trim();
    ...
    return [{ resultType: result.resultType || '', resultMsg: result.resultMsg || '', code: data.code ?? '', msg: data.msg || '' }];
  },
});
