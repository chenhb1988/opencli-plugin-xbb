...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/communicatePlan/add';
...
cli({
  ...
  args: [
    ...
    { name: 'dataList', type: 'str', help: '访客计划数据JSON字符串（必填）' },
    { name: 'formId', type: 'int', default: '', help: '表单模板id（可选）' },
  ...
  func: async function (kwargs) {
    ...
    return [{ dataId: result.dataId || '', resultCode: result.code ?? '', resultMsg: result.msg || '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
  },
});
