...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/workReportMonthly/add';
...
function makeErrorRow(code, msg) {
  return [{ dataId: '', code, msg }];
}

cli({
  site: 'xbb',
  name: 'work-report-monthly-add',
  description: '新建月报接口（businessType: 2104）',
  strategy: Strategy.PUBLIC,
  access: 'write',
  browser: false,
  domain: 'proapi.xbongbong.com',
  args: [
    { name: 'dataList', type: 'str', help: '月报数据JSON字符串（必填）' },
  ...
  func: async function (kwargs) {
    ...
    if (!payload.userId) return makeErrorRow('NO_USERID', '缺少 --userId');
    ...
    return [{ dataId: data.result?.dataId || '', code: data.code ?? '', msg: data.msg || '' }];
  },
});
