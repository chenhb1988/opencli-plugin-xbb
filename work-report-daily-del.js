...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/workReportDaily/del';
...
function makeErrorRow(code, msg, debug, body = '', responseBody = '') {
  return [{ dataId: '', errorDataMemo: '', code, msg, requestBody: debug ? body : '', responseBody: debug ? responseBody : '' }];
}

cli({
  site: 'xbb',
  name: 'work-report-daily-del',
  description: '删除日报接口',
  ...
  columns: ['dataId', 'errorDataMemo', 'code', 'msg', 'requestBody', 'responseBody'],
  func: async (kwargs) => {
    ...
    return [{ dataId: payload.dataId, errorDataMemo: data.result?.errorDataMemo || '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? body : '', responseBody: debug ? responseBody : '' }];
  },
});
