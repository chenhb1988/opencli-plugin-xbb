...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/customer/invoice/infoEdit';
...
function makeErrorRow(code, msg, debug, requestBody = '', responseBody = '') {
  return [{ invoiceId: '', resultCode: '', resultMsg: '', code, msg, requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
}
...
cli({
  ...
  args: [
    ...
    { name: 'invoiceId', type: 'int', help: '开票信息id（必填）' },
  ...
  func: async function (kwargs) {
    ...
    const payload = { corpid: String(kwargs.corpid || ''), customerId: Number(kwargs.customerId || 0), invoiceId: Number(kwargs.invoiceId || 0) };
    ...
    if (!payload.invoiceId) return makeErrorRow('NO_INVOICEID', '缺少 --invoiceId', debug, requestBody, '');
    ...
    return [{ invoiceId: result.invoiceId || payload.invoiceId || '', resultCode: result.code ?? '', resultMsg: result.msg || '', code: data.code ?? '', msg: data.msg || '', requestBody: debug ? requestBody : '', responseBody: debug ? responseBody : '' }];
  },
});
