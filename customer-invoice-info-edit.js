...
function makeErrorRow(code, msg) {
  return [{ invoiceId: '', resultCode: '', resultMsg: '', code, msg }];
}
...
cli({
  ...
  columns: ['invoiceId', 'resultCode', 'resultMsg', 'code', 'msg'],
  func: async function (kwargs) {
    ...
    if (!payload.invoiceId) return makeErrorRow('NO_INVOICEID', '缺少 --invoiceId');
    ...
    return [{ invoiceId: result.invoiceId || payload.invoiceId || '', resultCode: result.code ?? '', resultMsg: result.msg || '', code: data.code ?? '', msg: data.msg || '' }];
  },
});
