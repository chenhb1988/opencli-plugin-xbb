...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/paySheet/del';
...
function makeErrorRow(code, msg) {
  return [{ resultCode: '', resultMsg: '', errorDataMemo: '', code, msg }];
}

cli({
  site: 'xbb',
  name: 'pay-sheet-del',
  description: '删除付款单接口（businessType: 802）',
  ...
  func: async (kwargs) => {
    ...
    return [{ resultCode: data.code ?? '', resultMsg: data.msg || '', errorDataMemo: result.errorDataMemo || '', code: data.code ?? '', msg: data.msg || '' }];
  },
});
