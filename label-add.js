...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/label/add';
...
cli({
  site: 'xbb',
  name: 'label-add',
  description: '新建标签接口（businessType: -1；label 模块）',
  ...
  args: [
    ...
    { name: 'formId', type: 'str', default: '', help: '表单id（可选，接口示例中会携带）' },
    { name: 'groupId', type: 'int', help: '分组id（必填）' },
    { name: 'name', type: 'str', help: '标签名称（必填）' },
    { name: 'color', type: 'str', help: '标签颜色，如 #F3B563（必填）' },
  ...
  func: async function (kwargs) {
    ...
    if (String(kwargs.formId ?? '') !== '') payload.formId = Number(kwargs.formId);
    payload.groupId = Number(kwargs.groupId || 0);
    payload.name = String(kwargs.name || '').trim();
    payload.color = String(kwargs.color || '').trim();
    ...
    if (!payload.color) return makeErrorRow('NO_COLOR', '缺少 --color');
