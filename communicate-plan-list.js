...
cli({
  ...
  func: async (kwargs) => {
    ...
    const payload = { formId: Number(kwargs.formId || 0), page: Number(kwargs.page || 1), pageSize: Number(kwargs.pageSize || 20), corpid: String(kwargs.corpid || '') };
    ...
    return list.slice(0, Number(kwargs.limit || 20)).map((item, index) => ({ rank: index + 1, dataId: item.dataId || '', formId: item.formId || '', name: item.data?.text_1 || '', customer: item.data?.text_2 || '', remindType: item.data?.text_5 || '', visitType: item.data?.text_4 || '', visitTime: item.data?.date_1 || '', addTime: item.addTime || '', updateTime: item.updateTime || '', data: JSON.stringify(item.data || {}), code: '', msg: '', requestBody: debug ? body : '', responseBody: '' }));
  },
});
