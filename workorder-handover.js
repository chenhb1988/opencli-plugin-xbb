...
cli({
  ...
  func: async (kwargs) => {
    ...
    if (String(kwargs.retainMainUser ?? '') !== '') payload.retainMainUser = Number(kwargs.retainMainUser);
    if (kwargs.transfer) payload.transfer = String(kwargs.transfer);
    ...
    if (!payload.formId) {
      return makeErrorRow('NO_FORMID', '缺少 --formId');
    }
