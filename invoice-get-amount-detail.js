...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/invoice/getAmountDetail';
...
function makeErrorRow(code, msg) {
  return [{ rank: '', invoiceAmount: '', sumAmount: '', minInvoiceAmount: '', maxInvoiceAmount: '', contract: '', customer: '', code, msg }];
}

cli({
  site: 'xbb',
  name: 'invoice-get-amount-detail',
  description: '销项发票金额明细接口（businessType: 901）',
  strategy: Strategy.PUBLIC,
  access: 'read',
  browser: false,
  domain: 'proapi.xbongbong.com',
  args: [
    { name: 'dataId', type: 'int', help: '销项发票id（必填）' },
    { name: 'userId', type: 'str', default: '', help: '操作人id（可选）' },
    { name: 'debug', type: 'bool', default: false, help: '输出请求体和返回体调试信息' },
  ],
  columns: ['rank', 'invoiceAmount', 'sumAmount', 'minInvoiceAmount', 'maxInvoiceAmount', 'contract', 'customer', 'code', 'msg'],
  func: async function (kwargs) {
    const debug = Boolean(kwargs.debug);
    const { corpid, token, baseUrl, userId } = getRuntimeConfig();
    const runtimeUserId = String(kwargs.userId || userId || '').trim();
    const payload = { corpid };
    payload.dataId = Number(kwargs.dataId || 0);
    ...
    const list = Array.isArray(data.result?.amountDetail) ? data.result.amountDetail : [];
    if (!list.length) return makeErrorRow('NO_DATA', '接口成功，但 amountDetail 为空');
    return list.map((item, index) => ({ rank: index + 1, invoiceAmount: item.invoiceAmount ?? '', sumAmount: item.sumAmount ?? '', minInvoiceAmount: item.minInvoiceAmount ?? '', maxInvoiceAmount: item.maxInvoiceAmount ?? '', contract: JSON.stringify(item.contract || []), customer: JSON.stringify(item.customer || []), code: '', msg: '' }));
  },
});
