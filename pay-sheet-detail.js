...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/paySheet/detail';
...
function makeErrorRow(code, msg) {
  return [{ dataId: '', formId: '', addTime: '', updateTime: '', alone: '', uuid: '', data: '', code, msg }];
}

cli({
  site: 'xbb',
  name: 'pay-sheet-detail',
  description: '付款单详情接口（businessType: 802）',
  ...
  args: [
    ...
    { name: 'queryFlag', type: 'str', default: '', help: '是否查询审批数据：0非审批数据，1审批数据，2不论是否审批中都查询，默认0（可选）' },
    { name: 'debug', type: 'bool', default: false, help: '输出请求体和返回体调试信息' },
  ],
  columns: ['dataId', 'formId', 'addTime', 'updateTime', 'alone', 'uuid', 'data', 'code', 'msg'],
  func: async (kwargs) => {
    ...
    if (!token) return makeErrorRow('NO_TOKEN', '缺少 token；请先执行 xbbcli token-set --corpid <CORPID> --token <TOKEN> --userId <USERID>');
    ...
    return [{ dataId: result.dataId || '', formId: result.formId || '', addTime: result.addTime || '', updateTime: result.updateTime || '', alone: result.alone || '', uuid: result.uuid || '', data: JSON.stringify(result.data || {}), code: data.code ?? '', msg: data.msg || '' }];
  },
});
