...
function makeErrorRow(code, msg, debug, body = '', responseBody = '') {
  return [{ dataId: '', formId: '', addTime: '', updateTime: '', alone: '', uuid: '', data: '', code, msg, requestBody: debug ? body : '', responseBody: debug ? responseBody : '' }];
}
...
cli({
  ...
  args: [
    ...
    { name: 'queryFlag', type: 'int', default: '', help: '是否查询审批数据：0非审批数据，1审批数据，2不论是否审批中都查询，默认0（可选）' },
    { name: 'debug', type: 'bool', default: false, help: '输出请求体和返回体调试信息' },
  ],
  columns: ['dataId', 'formId', 'addTime', 'updateTime', 'alone', 'uuid', 'data', 'code', 'msg', 'requestBody', 'responseBody'],
  func: async (kwargs) => {
    ...
    return [{ dataId: result.dataId || '', formId: result.formId || '', addTime: result.addTime || '', updateTime: result.updateTime || '', alone: result.alone || '', uuid: result.uuid || '', data: JSON.stringify(result.data || {}), code: data.code ?? '', msg: data.msg || '', requestBody: debug ? body : '', responseBody: debug ? responseBody : '' }];
  },
});
