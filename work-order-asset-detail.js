...
const API_URL = 'https://proapi.xbongbong.com/pro/v2/api/workOrderV2/asset/detail';
...
cli({
  site: 'xbb',
  name: 'work-order-asset-detail',
  description: '资产详情接口（businessType: -1；workOrderV2 资产）',
  ...
  args: [
    ...
    { name: 'queryFlag', type: 'str', default: '', help: '是否查询审批数据，0非审批数据,1审批数据,2不论数据是否审批中都查询（可选）' },
  ...
  func: async function (kwargs) {
    ...
    return [{ rank: 1, dataId: result.dataId || '', formId: result.formId || '', addTime: result.addTime || '', updateTime: result.updateTime || '', data: JSON.stringify(result.data || {}), code: data.code ?? '', msg: data.msg || '' }];
  },
});
